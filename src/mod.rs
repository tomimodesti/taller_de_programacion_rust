...
mod parseo;
use parseo::{parseo_comando, decidir_comando};
mod archivo;
use archivo::{crear_archivo, buscar_archivo, escrbir_en_archivo, abrir_para_appendear};
mod comandos;
use comandos::{Comando, ejecutar_set, ejecutar_get, ejecutar_delete, ejecutar_length, ejecutar_snapshot};
...
pub fn main_tp() -> () {
    ...
    hashMap = cargar_hashmap(DATA_PATH, hashMap); //cargamos data
    hashMap = cargar_hashmap(LOG_PATH, hashMap); //cargamos log
    ...
    match comando { del parseo del comando
        Ok(comando_valido) => {
            let resultado = comando_valido.ejecutar();
     ...
     */
    //let resultado = comando.ejecutar() --> deberia ser un comando el resultado del parseo
    // match resultado --> y devolveria dos mensajes (mesnaje de resultado o de error) 
    println!("Resultado del parseo: {:?}", comando);
}
